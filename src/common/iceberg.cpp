...
namespace duckdb
{
	...
	string IcebergSnapshot::ReadMetaData(const string &path, FileSystem &fs)
	{
		...
		else
		{
			...
			metadata_file_path = fs.JoinPath(meta_path, table_version + ".metadata.json");
		}
	...
	string IcebergSnapshot::GetTableVersion(const string &path, FileSystem &fs)
	{
		...
		else
		{
			...
			fs.ListFiles(meta_path, [&](const string &fname, bool is_dir)
									 {
			if (StringUtil::EndsWith(fname, ".metadata.json")) {
				  auto full_path = fname;
					auto file_name = full_path.replace(0, meta_path.size(), "");
					auto version = file_name.substr(1, file_name.size() - 15);
					std::cout << version <<"\n";
			...
			{
				throw IOException("No metadata file found in " + path);
			}
			else
			{
